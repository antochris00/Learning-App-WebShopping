...
export class CustomerComponent implements OnInit {
  ...
  getCustomerAddress(cust:any){
    // if(cust.customerAddress!==undefined){
    //   let address[] = cust.customerAddress[0];
    //    return (address.doorNumber+", "+address.street+", "+address.city+"-"+address.pincode);
    // }
  ...
  slidetoggle(){
      ...
      this.customerService.changeCustomerStatus(this.customer)
                        .subscribe((resp:any) => {
                        if(resp.statusCode === 200){
                          (this.customer.active === true) ?
                          this.alertService.success('Customer Account is Activated', this.alertoptions)
                          : this.alertService.warn('Customer Account is Locked' , this.alertoptions);
                        }
